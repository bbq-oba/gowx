...
func (self *wxweb) handleMsg(r interface{}) {
	...
	for _, msg := range r.(map[string]interface{})["AddMsgList"].([]interface{}) {
		...
		toUserName := msg.(map[string]interface{})["ToUserName"].(string)
		// name = self.getUserRemarkName(msg['FromUserName'])
		content := msg.(map[string]interface{})["Content"].(string)
		content = strings.Replace(content, "&lt;", "<", -1)
		content = strings.Replace(content, "&gt;", ">", -1)
		content = strings.Replace(content, " ", " ", 1)
		utils.Display("msgType", msgType, "fromUserName", fromUserName, "content", content, "ToUserName", toUserName, "fromUserName[:2]", fromUserName[:2])
...
func (self *wxweb) webwxsendmsg(message string, toUseNname string) bool {
	...
	utils.Display("data", data, "err", err)
	if err != nil {
		debugPrint(err)
		return false
	} else {
